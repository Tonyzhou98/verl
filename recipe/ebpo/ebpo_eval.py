...
def main():
    ...
    if "aime" in args.test_file or "amc" in args.test_file:
        df = pd.read_parquet(args.test_file)
        repeat_number = 16
    elif "olympiadbench" in args.test_file:
        df = pd.read_parquet(args.test_file)
        repeat_number = 4
    elif "math_500" in args.test_file:
        # read jsonl file
    ...
    prompt_prehend = "Solve the following math problem step by step. The last line of your response should be of the form Answer: $Answer (without quotes) where $Answer is the answer to the problem.\n\n"

    # calcualte the mean of avg 16 runs
    accuracy_list = []
    for i in range(repeat_number):
        ...
        for _, row in df.iterrows():
            if "aime_2024" in args.test_file:
                prompt_list.append(prompt_prehend + row["Problem"])
                ground_truth.append(str(row["Answer"]))
            elif "aime_2025" in args.test_file:
                prompt_list.append(prompt_prehend + row["problem"])
                ground_truth.append(str(row["answer"]))
            elif "math_500" in args.test_file:
                prompt_list.append(prompt_prehend + row["problem"])
                ground_truth.append(str(row["answer"]))
            elif "amc23" in args.test_file:
                prompt_list.append(prompt_prehend + row["question"])
                ground_truth.append(str(row["answer"]))
            elif "olympiadbench" in args.test_file:
                prompt_list.append(prompt_prehend + row["question"])
                ground_truth.append(str(row["final_answer"][0]))
        ...
        for pred, answer in zip(predictions, ground_truth):
            ...
            acc += result["acc"]
            # print("pred: ", pred, "answer: ", answer, "acc: ", result["acc"])
            count += 1
    ...
    print(
        f"model name: {args.model_path}, dataset: {args.test_file} mean accuracy of {repeat_number} runs: {sum(accuracy_list) / len(accuracy_list)}"
    )
